...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut(confPath, '/tmp/pulsewatch.conf', async (e) => {
      ...
      await sudo(conn, "grep -q 'HOSTNAME=127.0.0.1' /etc/systemd/system/pulsewatch-web.service || sed -i 's/HOSTNAME=0.0.0.0/HOSTNAME=127.0.0.1/' /etc/systemd/system/pulsewatch-web.service");
...
}).connect({ host: '49.234.112.108', port: 22, username: 'ubuntu', password: PASS, readyTimeout: 20000 });
