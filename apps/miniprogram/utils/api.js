...
function register(email, password, displayName) {
  return request('/api/v1/auth/register', {
    method: 'POST',
    auth: false,
    data: { email: email, password: password, displayName: displayName || '', otpCode: '' },
  }).then(function (data) {
    if (!data.accessToken) {
...
function addIncidentNote(incidentId, message) {
...
function getStatusPages() {
  return request(orgPath('/status-pages'));
}

function getStatusPage(id) {
  return request(orgPath('/status-pages/' + id));
}
...
function getStatusPageAnnouncements(pageId) {
  return request(orgPath('/status-pages/' + pageId + '/announcements'));
}

function createAnnouncement(pageId, data) {
  return request(orgPath('/status-pages/' + pageId + '/announcements'), {
    method: 'POST',
    data: data,
  });
...
function deleteAnnouncement(pageId, announcementId) {
  return request(orgPath('/status-pages/' + pageId + '/announcements/' + announcementId), {
    method: 'DELETE',
  });
}
