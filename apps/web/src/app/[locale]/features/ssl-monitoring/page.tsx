...
import { Activity, Bell, Calendar, Lock, Shield } from 'lucide-react';
import { buildPageMetadata } from '@/lib/seo';

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'meta.sslMonitoring' });
  return buildPageMetadata({
    locale,
    path: '/features/ssl-monitoring',
    title: t('title'),
...
export default async function SslMonitoringFeaturePage() {
  const t = await getTranslations('sslMonitoring');

  const highlights = [
    { icon: Lock, title: t('h1Title'), desc: t('h1Desc') },
    { icon: Calendar, title: t('h2Title'), desc: t('h2Desc') },
    { icon: Shield, title: t('h3Title'), desc: t('h3Desc') },
