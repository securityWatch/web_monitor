...
import { Activity, Globe, Megaphone, Radio, Wrench } from 'lucide-react';
import { buildPageMetadata } from '@/lib/seo';

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'meta.statusPages' });
  return buildPageMetadata({
    locale,
    path: '/features/status-pages',
    title: t('title'),
...
export default async function StatusPagesFeaturePage() {
  const t = await getTranslations('statusPagesFeature');

  const highlights = [
    { icon: Radio, title: t('h1Title'), desc: t('h1Desc') },
    { icon: Megaphone, title: t('h2Title'), desc: t('h2Desc') },
    { icon: Wrench, title: t('h3Title'), desc: t('h3Desc') },
    { icon: Globe, title: t('h4Title'), desc: t('h4Desc') },
  ];
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-white">
      ...
      <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-12 flex flex-wrap gap-4">
          ...
          <Link href="/features/uptime-monitoring" className="btn-secondary">{t('ctaUptime')}</Link>
