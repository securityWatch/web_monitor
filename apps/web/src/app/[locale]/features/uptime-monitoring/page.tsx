...
import { Activity, Bell, Globe, LineChart, Shield } from 'lucide-react';
import { buildPageMetadata } from '@/lib/seo';

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'meta.uptimeMonitoring' });
...
export default async function UptimeMonitoringFeaturePage() {
  const t = await getTranslations('uptimeMonitoring');

  const highlights = [
    { icon: Globe, title: t('h1Title'), desc: t('h1Desc') },
    { icon: Shield, title: t('h2Title'), desc: t('h2Desc') },
    { icon: LineChart, title: t('h3Title'), desc: t('h3Desc') },
